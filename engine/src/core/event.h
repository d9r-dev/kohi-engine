 ...
 * listener/callback combos will ot be registered again and will cause this to return FALSE.
 * @param code The event code to listen for.
 * @param listener A pointer to a listeer instance. Can be NULL.
 * @param on_event The callback function pointer to be invoked when the event code is fired.
 * @returns TRUE if the event is successfully registered, FALSE if it is not.
 */
KAPI b8 event_register(u16 code, void *listener, PFN_on_event on_event);

/**
 * Unregister to stop listening for events with the provided code. If no matching registration is
 ...
 * @param on_event The callback function pointer to be invoked when the event code is fired.
 * @returns TRUE if the event is successfully unregistered, FALSE if it is not.
 */
 ...
 * @returns TRUE if handle, FALSE if it is not.
 */
