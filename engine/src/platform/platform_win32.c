    ...
    i32 height)
{
    ...
    // Setup and regiser window class.
    ...
    wc.hIcon = icon;
    wc.hCursor = LoadCursor(NULL, IDC_ARROW); // NULL; // Mange the cursor manually
    ...
    HWND handle = CreateWindowExA(
        ...
        0, 0, state->h_instance, NULL);

    if (!handle)
    {
        MessageBoxA(NULL, "window creation failed!", "Error", MB_ICONEXCLAMATION | MB_OK);

        KFATAL("Failed to create window!");
        return FALSE;
    }
    else
    {
    ...
    // If initially maximized, use SW_SHOWMAXIMIZED : SW_SHOWMAXNOACTIVATE;
...
LRESULT CALLBACK win32_process_message(HWND hwnd, u32 msg, WPARAM w_param, LPARAM l_param)
{
    ...
    {
    case WM_ERASEBKGND:
        // Notfiy the OS that erasing wil be handled by the application to prevent flicker.
        return 1;
    case WM_CLOSE:
        // TODO: Fire an eventfor the application to quit.
        return 0;
    case WM_DESTROY:
        PostQuitMessage(0);
        return 0;
